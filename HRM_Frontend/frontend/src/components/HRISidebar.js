...
export default function HRISidebar({ isCollapsed = false, setIsCollapsed }) {
  ...
  return (
    <aside 
      className={`bg-white h-screen flex flex-col fixed left-0 top-0 border-r border-gray-100/80 z-50 select-none font-sans overflow-x-hidden transition-all duration-300 ease-in-out ${
        isCollapsed 
          ? 'w-[75px]' 
          : 'w-[290px] max-md:shadow-[5px_0_25px_rgba(0,0,0,0.08)]'
      }`}
    >
      {/* Sidebar Header Branding and Toggle Button */}
      ...
      {/* Navigation Links Group Mapping */}
            ...
            <div key={idx} className="w-full relative group">
              <button
                onClick={() => {
                  ...
                  // Close the floating drawer menu automatically on mobile *only* if selecting a direct page route
